...
Built on NixOS under WSL and cross-compiled to Windows. The `flake.nix` dev
shell provides the Rust toolchain with the `x86_64-pc-windows-gnu` target,
the mingw cross-linker, and the analysis tools (python, binutils, Ghidra).
...
inside the hook. The reverse-engineering record is in `docs/` and the
reference mod's decompile is regenerated by `tools/ghidra/`.
