...
impl InitializationContext {
    ...
    // except a swap chain whose identity differs from the stored one: that one
    // replaces it. Keeping the old `IDXGISwapChain3` around would keep a swap
    // chain alive that the game has already replaced, and DXGI refuses to
    // create a new swap chain for an HWND that still has a live one.
    fn insert_swap_chain(&mut self, swap_chain: &IDXGISwapChain3) {
        *self = match mem::replace(self, InitializationContext::Empty) {
            ...
            InitializationContext::WithSwapChain(stored) => {
                if same_swap_chain(&stored, swap_chain) {
                    InitializationContext::WithSwapChain(stored)
                } else {
                    debug!("Replacing stale captured swap chain {stored:?} with {swap_chain:?}");
                    InitializationContext::WithSwapChain(swap_chain.clone())
                }
            },
            InitializationContext::Complete(stored, command_queue) => {
                if same_swap_chain(&stored, swap_chain) {
                    InitializationContext::Complete(stored, command_queue)
                } else {
                    // The command queue was captured for the old swap chain, so
                    // it is dropped with it; the queue for the new one comes
                    // back through ExecuteCommandLists or swap-chain creation.
                    debug!("Replacing stale captured swap chain {stored:?} with {swap_chain:?}");
                    InitializationContext::WithSwapChain(swap_chain.clone())
                }
            },
    ...
    // Whether a strong reference to a swap chain is currently held.
    fn holds_swap_chain(&self) -> bool {
        matches!(
            self,
            InitializationContext::WithSwapChain(_) | InitializationContext::Complete(..)
        )
    }
...
impl InitState {
    ...
    fn holds_swap_chain(&self) -> bool {
        self.inner.lock().holds_swap_chain()
    }
...
// Two interface pointers name the same object iff their `IUnknown` identities
// match. A failed query is reported as "different" so the caller replaces what
// it holds rather than keeping something it can no longer identify.
fn same_swap_chain(lhs: &IDXGISwapChain3, rhs: &IDXGISwapChain3) -> bool {
...
// Whether hudhook currently holds anything that keeps a swap chain alive: the
// live pipeline, the active context, or a captured swap chain in the
// initialization state machine.
unsafe fn holds_swap_chain_references() -> bool {
    ...
    INIT_STATE.holds_swap_chain()
}

// Drop every reference hudhook holds before the game performs an operation DXGI
// validates against the swap chain's reference count: `ResizeBuffers`, and
// creating a second swap chain for a window that already has one (which fails
// with E_ACCESSDENIED while any reference is alive, ours included).
//
...
unsafe extern "system" fn dxgi_swap_chain_resize_buffers_impl(
    ...
    release_swap_chain_references("ResizeBuffers");
...
unsafe extern "system" fn dxgi_swap_chain_set_source_size_impl(
    ...
    release_swap_chain_references("SetSourceSize");
...
unsafe extern "system" fn dxgi_swap_chain_resize_buffers1_impl(
    ...
    release_swap_chain_references("ResizeBuffers1");
...
unsafe extern "system" fn dxgi_factory_create_swap_chain_impl(
    ...
    release_swap_chain_references("swap-chain creation");
