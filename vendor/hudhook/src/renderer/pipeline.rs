...
impl<T: RenderEngine> Pipeline<T> {
    ...
    pub(crate) fn update_display_size_from_swap_chain(&mut self, width: u32, height: u32) {
        if width > 0 && height > 0 {
            ...
            // `display_size` IS the back buffer here, so the framebuffer scale is
            // 1 by definition. Upstream set it to the window's DPI scale, which
            // made the viewport (display_size * scale) larger than the back
            // buffer while the scissor rects stayed unscaled: at 125% Windows
            // scaling the UI drew 1.25x too large and was clipped to 80% of
            // itself. DPI belongs in font/style scaling, which the render loop
            // owns.
