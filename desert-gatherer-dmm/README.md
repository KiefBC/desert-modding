...
3. **Structural check (catches the failure mode that silently breaks `rebase.py`, before spending time on it).** Run
   ...
   - **Step 1 is a precondition, not an option: if the clean table is absent the cross-check passes by skipping**, printing a banner saying the oracle did not run and the run proved nothing. Do not read that `ok` as a structural check — it never happened.
   - Nothing in the cross-check is pinned to a game build any more. It locates every record by key and name and compares `record_rel_offset` (counted from each record's `u32` key) rather than absolute offsets, so it runs against any build's clean table with no constants to update and without the pack being rebased first. That is what makes it usable as a pre-check here. `LOADER_RVA` is still build-pinned, but it belongs to the separate record-loader test in the same file, which prints the real resolved RVA before failing; update it there.
